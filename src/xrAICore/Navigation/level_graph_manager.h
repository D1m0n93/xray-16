...
public:
    ...
    {
        if (version == XRAI_VERSION_BIG)
        {
            m_nodes = static_cast<CLevelVertex*>(stream->pointer());
        }
        //else if (version == XRAI_VERSION_COP || version == XRAI_VERSION_CS)
        //{
        //    m_compatibility_mode = true;
        //    m_nodes = new CLevelVertex[vertex_count + 1]; // additional one, so we don't trigger access violation
        //    NodeCompressed10* oldNodes = static_cast<NodeCompressed10*>(stream->pointer());

        //    for (size_t i = 0; i < vertex_count; ++i)
        //    {
        //        NodeCompressed10& oldNode = oldNodes[i];
        ...
        //    NodeCompressed& endNode = m_nodes[vertex_count];
        //    endNode.data[0]  = 'E';
        //    endNode.data[1]  = 'N';
        //    endNode.data[2]  = 'D';
        //    endNode.data[3]  = ' ';
        ...
        //    endNode.data[11] = 'C';
        //    endNode.data[12] = ' ';
        //    static_assert(sizeof(endNode.data) == 13, "If you have changed the NodeCompressed structure, please update the code above.");
        //}
        else
        {
